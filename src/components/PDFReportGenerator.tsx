...
const PDFReportGenerator: React.FC<PDFReportGeneratorProps> = ({ 
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors ${className}`}
    >
      {isGenerating ? (
        <>
          ...
          {language === 'en-US' ? 'Generating...' : language === 'zh-HK' ? '生成中...' : '生成中...'}
        </>
      ) : (
        <>
          ...
          <span className="flex-shrink-0">
            {language === 'en-US' ? 'Download Report' : language === 'zh-HK' ? '下載報告' : '下载报告'}
          </span>
        </>
      )}
    </button>
